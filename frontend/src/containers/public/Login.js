...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(validatiton).length > 0) {
      ...
      axios.post('http://localhost:5000/signUp',payloadRegister)
        .then(
          res=>Swal.fire({
      ...
      <div className="w-1124 m-auto flex justify-center flex-row">
        <div className="mt-[130px] w-[500px] bg-white drop-shadow-xl py-10 px-5 font-sans">
          ...
          <form onSubmit={handleSubmit}>
            ...
            />
            {invalidfield.email && (
              <p style={{ color: "red" }}>{invalidfield.email}</p>
            )}
            <InputForm
              setInvalidfield={setInvalidfield}
              text="Địa chỉ"
            ...
            {invalidfield.address && (
              <p style={{ color: "red" }}>{invalidfield.address}</p>
            )}
            <InputForm
              setInvalidfield={setInvalidfield}
              text="SĐT"
            ...
            />
            {invalidfield.phone && (
              <p style={{ color: "red" }}>{invalidfield.phone}</p>
            )}
            <InputForm
              setInvalidfield={setInvalidfield}
              text="Tên đăng nhập"
            ...
            />
            {invalidfield.repass && (
              <p style={{ color: "red" }}>{invalidfield.repass}</p>
            )}
